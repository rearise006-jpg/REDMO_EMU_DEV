    ...
    public sealed partial class MapServer
    {
        private void DropsOperation(GameMap map)
        {
            try
            {
                ...
                if (totalTime >= 1000)
                    _logger.Warning($"DropsOperation taked too much time to read: {totalTime}!!");
            }
            catch (Exception ex)
            {
                _logger.Error($"[MapServerDropsOperation] :: {ex.Message}");
                _logger.Verbose($"[MapServerDropsOperation] :: {ex.InnerException}");
                _logger.Verbose($"[MapServerDropsOperation] :: {ex.StackTrace}");
            }
        }
