...
namespace DigitalWorldOnline.Account
{
    public sealed class AuthenticationServer : GameServer, IHostedService
    {
        ...
        public AuthenticationServer(IHostApplicationLifetime hostApplicationLifetime, IConfiguration configuration, IProcessor processor, ILogger logger, ISender sender)
        {
        ...
        {
            Console.WriteLine();
            LogMessage(ConsoleColor.Green, $"CONNECTING ON IP: [{_configuration[AuthenticationServerAddress]}:{_configuration[AuthenticationServerPort]}]");

            if (!Listen(_configuration[AuthenticationServerAddress], _configuration[AuthenticationServerPort], _configuration[AuthenticationServerBacklog]))
            {
                LogTitleMessage(ConsoleColor.Red, $"FAILED TO CONNECT ON IP: [{_configuration[AuthenticationServerAddress]}:{_configuration[AuthenticationServerPort]}]");

                _hostApplicationLifetime.StopApplication();
                return;
            }

            LogTitleMessage(ConsoleColor.Green, "AUTH SERVER STARTED");
            Console.WriteLine();
        }
        ...
        {
            LogTitleMessage(ConsoleColor.Red, "STOPPING AUTH SERVER");
        ...
        {
            LogTitleMessage(ConsoleColor.Red, "AUTH SERVER STOPPED");
        }
