...
namespace DigitalWorldOnline.Commons.Packets.GameServer
{
    public class DigimonArchiveLoadPacket : PacketWriter
    {
        ...
        public DigimonArchiveLoadPacket(CharacterDigimonArchiveModel digimonArchive, List<CharacterDigimonGrowthSystemModel> growthDigimons)
        {
            Type(PacketNumber);
            WriteInt(0);
            ...
            foreach (var digimonArchiveItem in digimonArchive.DigimonArchives.Where(x => x.Digimon != null))
            {
                ...
                bool hasAnySkills2 = digimon.Evolutions.Any(evo => evo.SkillsMemory.Count > 0);

                if (hasAnySkills2)
                {
                    var evolutionsWithSkills = digimon.Evolutions.Where(evo => evo.SkillsMemory.Any()).ToList();
