...
namespace DigitalWorldOnline.Commons.Packets.GameServer
{
    public class InitialInfoPacket : PacketWriter
    {
        ...
        public InitialInfoPacket(CharacterModel character, GameParty? party, byte masterMatchTeam, MapObjectAssetModel mapObject)
        {
            Type(PacketNumber);
            WriteInt(1);
            ...
            //Console.WriteLine($"Initial Info | SkillMemory Count {character.Partner.SkillMemory.Count}");
            WriteInt(character.Partner.GeneralHandler);
            // Verifica se há skills em QUALQUER evolução
            bool hasAnySkills = character.Partner.Evolutions.Any(evo => evo.SkillsMemory.Count > 0);

            if (hasAnySkills)
            {
                // Filtra apenas evoluções com skills do tipo correspondente
                var evolutionsWithSkills = character.Partner.Evolutions
                    .Where(evo => evo.SkillsMemory.Any())
                    .ToList();

                // Envia a quantidade de evoluções com skills válidas
                WriteByte((byte)evolutionsWithSkills.Count);

                // Envia cada evolução com skills
                foreach (var evo in evolutionsWithSkills)
                {
                    if (evo.SkillsMemory.Count > 0)
                    {
                        WriteBytes(evo.ToArrayMemory(evo.SkillsMemory));
                    }
                }
            }
            else
            {
                WriteByte(0); // Nenhuma evolução tem skills
            }
            ...
            if (party != null)
            {
                ...
                foreach (var member in party.Members)
                {
                    ...
                    bool isInSameMap = party.Members.Values.All(m =>
                        m.Location.MapId == member.Value.Location.MapId &&
                        m.Channel == member.Value.Channel
                    );
                    ...
                    WriteInt((int)member.Value.Model);
            ...
            WriteInt(character.ClientOption); //clientOption (aparenta ter relação com o tutorial)
            ...
            WriteByte(masterMatchTeam); //master match (0 = None, 1 = equipe A, 2 = equipe B)

            WriteInt(character.CurrentActiveDeck ?? 0);
            ...
            {
                WriteInt(0);
                //WriteInt(mapObject.Size);

                //for (var i = 0; i < mapObject.Size; i++)
                //{
                //    WriteInt(mapObject.MapSourceObjects[i].ObjectId);
                //}
            }
