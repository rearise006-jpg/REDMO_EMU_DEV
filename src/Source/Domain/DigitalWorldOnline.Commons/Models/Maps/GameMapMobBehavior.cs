...
namespace DigitalWorldOnline.Commons.Models.Map
{
    public sealed partial class GameMap
    {
        ...
        public void UpdateMapMobs(List<KillSpawnConfigModel> targetMobs)
        {
            var mobsToRemove = new List<MobConfigModel>();

            foreach (var mob in Mobs)
                CheckRemoveMob(mobsToRemove, mob, targetMobs);
        ...
        private void CheckRemoveMob(List<MobConfigModel> mobsToRemove, MobConfigModel mob, List<KillSpawnConfigModel> targetMobs)
        {
            var targetKillSpawns = targetMobs.Where(x => x.TargetMobs.Any(mobConfigModel => mobConfigModel.TargetMobType == mob.Type)).ToList();

            if (targetKillSpawns == null || !targetKillSpawns.Any())
                return;

            foreach (var targetKillSpawn in targetKillSpawns)
            {
                foreach (var target in targetKillSpawn.TargetMobs.Where(x => x.TargetMobType == mob.Type).ToList())
                {
                    mob.SetCanSpawn();
                }
            }
        ...
        public void AttackTarget(MobConfigModel mob, MonsterSkillInfoAssetModel? targetSkill, List<NpcColiseumAssetModel> npcAsset, long currentTick)
        {
            ...
            var checkTamerWithAgroBuff = mob.TargetTamers.FirstOrDefault(tamer => tamer.BuffList.ActiveBuffs.Any(buff =>
            buff.BuffInfo.SkillInfo.Apply.Any(apply => apply.Attribute == Enums.SkillCodeApplyAttributeEnum.PROVOKE)));

            if (checkTamerWithAgroBuff != null && mob.TargetHandler != checkTamerWithAgroBuff.GeneralHandler)
                mob.SetAgroTarget(checkTamerWithAgroBuff.GeneralHandler);
                ...
                case 27045:
                    {
                        ...
                        foreach (var target in targetTamersCopy)
                        {
                            ...
                            {
                                // Gather effect targets for the skill
                                var effectTargets = new List<(uint, int, int)>();

                                foreach (var tamer in targetTamers)
                                {
                                    effectTargets.Add(((uint)target.Partner.Id, target.Partner.Location.X, target.Partner.Location.Y));
                                }

                                // Send the visual packet
                                BroadcastForTargetTamers(mob.TamersViewing,
                                    new MonsterSkillVisualPacket((uint)mob.GeneralHandler, (uint)targetSkill.SkillId, effectTargets).Serialize());

                                BroadcastForTargetTamers(mob.TamersViewing,
                                    new MonsterSkillDamagePacket(mob.GeneralHandler, targetSkill.SkillId, finalDamage, targetTamers).Serialize());
                ...
                case 27045:
                    {
                        ...
                        foreach (var target in targetTamersCopy)
                        {
                            ...
                            else
                            {
                                ...
                                BroadcastForTargetTamers(mob.TamersViewing, new MonsterSkillVisualPacket( (uint)mob.GeneralHandler, (uint)targetSkill.SkillId).Serialize());
        ...
        {
            var diff = UtilitiesFunctions.CalculateDistance(
                mob.CurrentLocation.X,mob.InitialLocation.X,
                mob.CurrentLocation.Y,mob.InitialLocation.Y);
            ...
            mob.UpdateChaseTime(DateTime.Now.AddMilliseconds(0));
        ...
        public void AttackNearbyTamer(MobConfigModel mob, List<long> nearbyTamers, List<NpcColiseumAssetModel> npcAsset)
        {
            ...
            {
                foreach (var tamerId in nearbyTamers.ToList())
                {
