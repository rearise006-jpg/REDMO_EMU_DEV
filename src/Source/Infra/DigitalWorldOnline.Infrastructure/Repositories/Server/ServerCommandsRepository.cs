...
namespace DigitalWorldOnline.Infrastructure.Repositories.Server
{
    public class ServerCommandsRepository : IServerCommandsRepository
    {
        ...
        public async Task UpdateDigimonDeckRewardAsync(DigimonModel digimon)
        {
            var dto = await _context.Digimon
                .AsNoTracking()
                .SingleOrDefaultAsync(x => x.Id == digimon.Id);

            if (dto != null)
            {
                dto.DeckRewardReceived = digimon.DeckRewardReceived;
        ...
        public async Task<MastersMatchRankerDTO> CreateMasterMatchPlayerCommandAsync(long characterId, string tamerName, MastersMatchTeamEnum assignedTeam)
        {
            var mastersMatch = await _context.MastersMatches.FirstOrDefaultAsync();

            if (mastersMatch == null)
            {
                throw new InvalidOperationException("MastersMatch parent record does not exist. Cannot create new ranker.");
            }

            var newRanker = new MastersMatchRankerDTO
            {
            ...
            _context.MastersMatchRankers.Add(newRanker);
